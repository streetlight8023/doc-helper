...
layui.define(["jquery", "laytpl"], function (exports) {
    ...
    var getAdvices = function(stackId){
        var advices = "";
         $.ajax({
            url: ctx + 'advice/list?pageNum=1&pageSize=10&stackId=' + stackId,
            async: false,
            type: 'get',
            success: function (res) {
                if(200 === res.code){
                    ...
                    for(var index in list){
                        var drugAdvice = list[index];
                        advices+=(drugAdvice.drugName+drugAdvice.perDose+drugAdvice.doseUnit+"   ");
                    }
                    console.log("final advices "+advices);
                }
            }
        });
         return advices;
    ...
    var getSelectDiv= function (x,y){

        var $primyDiagnosis = $("#primy-diagnosis-id"),
            $handleContent = $("#handle-content-id");
        var primyDiagnosisLeft =$primyDiagnosis.offset().left,
            primyDiagnosisTop  = $primyDiagnosis.offset().top,
            handleContentLeft = $handleContent.offset().left,
            handleContentTop  =  $handleContent.offset().top;

        console.log("x " +x+" y "+y+" stackInputLeft "+primyDiagnosisLeft+" stackInputTop "+primyDiagnosisTop+" stackNameLeft "+handleContentLeft+" stackNameTop "+handleContentTop);

        return Math.pow(primyDiagnosisLeft-x,2)+Math.pow(primyDiagnosisTop-y,2)>Math.pow(handleContentLeft-x,2)+Math.pow(handleContentTop-y,2)?$handleContent:$primyDiagnosis;
            ...
            options.draggable && options.elem.on("mousedown", ".eleTree-node-content", function (e) {
                ...
                $(document).on("mousemove", function (e) {
                    ...
                    if (target.parents(options.elem).length === 0 && !isTargetOuterMost) {
                        // $("#areaId").val(currentData.currentData.data.stackName);
                        // console.log("this x"+this.offset().left+" this y"+this.offset().top)
                        var el= getSelectDiv(e.clientX,e.clientY);
                        var advices =getAdvices(currentData.currentData.data.id);
                        el.val(el.val()+" "+ advices);
                        return;
                    }
                    // 判断初始与结束是否是同一个节点
                    ...
                    console.log("drag targetData "+targetData);
                    ...
                    console.log("是否最外层"+isCurrentOuterMost);
                    // 目标是否是最外层
                    isTargetOuterMost ? options.data.push(currData) : (function () {
                        ...
                        console.log("已经到最外层"+isCurrentOuterMost+"currData"+currData);
